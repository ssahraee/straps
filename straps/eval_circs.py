...
def eval_mul_both(p, pdts, d=2):
    ...
    x = ShLd(['out'], d)
    x.op('out', ['t0', 't1'], pdts['ISW'])
    return sum(x.distr()[i*2**d + 2**d-1] for i in range(2**d)) + np.sum(x.distr()[-2**d:-1])
...
base_circuits = {
        'eval_aes_sbox': (eval_aes_sbox, lambda **kwargs: ['ISW', 'simpleref', 'optref', 'square']),
        'eval_x_cube': (eval_x_cube, lambda **kwargs: ['ISW', 'simpleref', 'optref', 'square']),
...
specialized_circuits = {
    ...
    "aes_sbox_simpleref": ("AES S-box simple refresh", 'eval_aes_sbox', {"ref_name": "simpleref"}),
    "cube_isw_noref": ("ISW(x^2, x)", 'eval_x_cube', {"ref": False}),
    "cube_isw_optref": ("ISW(SNI-ref(x^2), x)", 'eval_x_cube', {"ref_name": "optref"}),
    "cube_isw_simpleref": ("ISW(simple-ref(x^2), x)", 'eval_x_cube', {"ref_name": "simpleref"}),
    "int_sq_isw_optref": ("single circuit ISW(x, opt-ref(x))",
        'eval_n_sharings', {'circ_name':'isw_sq_opt'}),
    "int_sq_isw_simpleref": ("single circuit ISW(x, simple-ref(x))",
        'eval_n_sharings', {'circ_name':'isw_sq_simple'}),
    "int_sq_isw_noref": ("single circuit ISW(x, x)",
        'eval_n_sharings', {'circ_name':'isw_sq_noref'}),
    "int_cube_isw_optref": ("single circuit ISW(x, opt-ref(x^2))",
        'eval_n_sharings', {'circ_name':'isw_cube_opt'}),
    "int_cube_isw_simpleref": ("single circuit ISW(x, simple-ref(x^2))",
        'eval_n_sharings', {'circ_name':'isw_cube_simple'}),
    "int_cube_isw_noref": ("single circuit ISW(x, x^2)",
        'eval_n_sharings', {'circ_name':'isw_cube_noref'}),
    "isw": ("ISW(x, y)",
            'eval_n_sharings', {'circ_name': 'ISW', 'n_inputs':2}),
    "isw_y": ("ISW(y, x)",
            'eval_n_sharings', {'circ_name': 'ISW', 'n_inputs':2, 'sec_input':1}),
    "isw_xy": ("x|y: ISW(y, x)", 'eval_mul_both', {}),
    "sh-add": ("Add(x, y)",
            'eval_n_sharings', {'circ_name': 'sharewise_add', 'n_inputs':2}),
}
...
def eval_circ_all_p(circ_name, k, e, d, ps, n_s_max, suff_thresh, use_copy, cum_tr):
    ...
    gpdts = {circ: ldt_sampling.gpdt(circ, d, k, e, n_s_max, suff_thresh, use_copy, cum_tr)
            for circ in base_circuits[f][1](**kwargs)}
    print("evaluating {} {}, d={}, n_s_max={}, suff_thresh={}, err={} use_copy={} cum_tr={}".format(
