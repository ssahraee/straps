...
def op_preamble(d, n_inputs):
    ...
    output_sh = [c.var("out_{}".format(i), kind='output', port=(0, i)) for i in range(d)]
    input_shs = [[c.var("in_{}_{}".format(j, i), kind='input', port=(j, i)) for i in range(d)]
...
def isw_inner(d, c, x, y, z):
    ...
    r = [{j: c.var('r_{}_{}'.format(i,j), kind='random') for j in range(i+1, d)}
            for i in range(d)]
    t = [{j: c.var('t_{}_{}'.format(i,j)) for j in range(i+1, d)}
            for i in range(d)]
    s = [[c.var('s_{}_{}'.format(i,j)) for j in range(d)]
            for i in range(d)]
    c_var = [[c.var('c_{}_{}'.format(i,j)) for j in range(d)]
            for i in range(d)]
    for i in range(d):
