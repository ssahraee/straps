...
class Circuit:
    ...
    def var(self, name, continuous=False, kind='intermediate', port=None,
            out_port=None):
    ...
    def sorted_vars(self):
        ...
        def copy_all_satisfying_predicate(pred):
            for i, v in enumerate(self.vars):
                if not copied_vars[i] and pred(i, v):
                    copy_var(i)
        for i, _ in sorted(((i, v) for i, v in enumerate(self.vars) if
            v.kind=='input'), key=lambda iv: self.input_ports[iv[1].idx]):
            copy_var(i)
        ...
        assert all(copied_vars), "Not copied: {}".format([v for i, v in enumerate(self.vars) if not
                copied_vars[i]])
    ...
    def assert_all_nodes_have_input(self):
        for v in self.vars:
            if v.kind == 'intermediate':
                assert [dest for dest, _ in
                        self.l_sums+self.l_prods+self.ngates+self.assigns if
                        dest.idx == v.idx], "{}".format(v)
    ...
    def var_mapped_new(self, map_new_to_olds, map_old_to_new, copy_assigns=False):
        ...
        for olds in map_new_to_olds:
            ...
            if any(self.vars[ov].kind == 'input' for ov in olds):
                kind = 'input'
                in_port = next(self.input_ports[ov] for ov in olds if self.vars[ov].kind == 'input')
            ...
            out_port = next((self.output_ports[ov] for ov in olds if ov in
                    self.output_ports), None)
            new_c.var(self.vars[next(iter(olds))].name, continuous, kind, in_port,
                    out_port)
    ...
    def to_comp_graph(self):
        ...
        sum_ops = [(dest, KIND_MAP["sum"], [op1.idx, op2.idx]) for dest, (op1, op2) in sc.l_sums]
        prod_ops = [(dest, KIND_MAP["product"], [op1.idx, op2.idx]) for dest, (op1, op2) in sc.l_prods]
        ...
        return PyCompGraph(comp_graph, in_ports, out_ports, self.d,
                len(sc.input_ports)//self.d, len(sc.output_ports)//self.d);
